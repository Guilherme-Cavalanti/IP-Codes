...
bool obterEmprestimo(CLIENTE* c1, int valor) {
    ...
    //ACIMA DA DÍVIDA MÁXIMA PARA CLIENTE ESPECIAL
    if(c1->tipo == 'E' && c1->valorDaDivida + valor > dividaMaximaEspecial) return false;
    //ACIMA DA DÍVIDA MÁXIMA PARA CLIENTE COMUM
	if(c1->tipo == 'C' && c1->valorDaDivida + valor > dividaMaxima) return false;
    //EMPRESTIMO É POSSÍVEL
...
bool pagarEmprestimo(CLIENTE* c1, int valor) {
	...
	if(valor<=0)return false;
    //VALOR MUITO ALTO
	if(c1->valorDaDivida< valor || c1->valorContaCorrente<valor)return false;
...
bool realizarSaque(CLIENTE* c1, int valor) {
    ...
    if(valor<=0)return false;
    //VALOR MUITO ALTO
    if(c1->valorContaCorrente<valor)return false;
	//SAQUE SENDO REALIZADO
    c1->valorContaCorrente -= valor;
	return true;
}
...
bool adicionarGerente(BANCO* b1, GERENTE* gerente){
    //NÃO CABEM MAIS GERENTES
	if (b1->numGerentes == 10) return false;
	//GERENTE REPETIDO
    int i;
	for (i=0;i<b1->numGerentes;i++){
		if (b1->gerentes[i]->pessoa.cpf == gerente->pessoa.cpf) return false;
	}
	//ADCIONANDO GERENTE
